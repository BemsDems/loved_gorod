...
class _CreateIssueScreenState extends State<CreateIssueScreen> {
  ...
  void _submit() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (mounted) {
        context.read<IssuesBloc>().add(IssuesCreateRequested(newIssue));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return BlocListener<IssuesBloc, IssuesState>(
      listenWhen:
          (previous, current) => previous.createStatus != current.createStatus,
      listener: (context, state) {
        if (state.createStatus == IssuesCreateStatus.success) {
          Navigator.pop(context);
        } else if (state.createStatus == IssuesCreateStatus.failure) {
          setState(() => _isLoading = false);
          AppSnackbars.showError(
            context,
            state.errorMessage ?? 'Ошибка при создании обращения',
          );
              ...
              child: ListView(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    validator:
                        (v) => v!.isEmpty ? 'Описание обязательно' : null,
                  ),
